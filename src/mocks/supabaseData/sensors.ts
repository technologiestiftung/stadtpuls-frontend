...
export const sensors: {
  ...
  withHttpIntegration: Omit<SensorQueryResponseType, "external_id">[];
} = {
  withTtnIntegration: [
    {
      id: 1,
...
export const parsedSensors: PublicSensorType[] = [
  ...sensors.withHttpIntegration.map(sensor => {
    return {
      ...sensor,
      authorName:
  ...
  ...sensors.withTtnIntegration.map(sensor => {
    return {
      ...sensor,
      authorName:
        userprofiles.find(profile => profile.id === sensor.user_id)
...
export const curatedSensors: PublicSensorType[] = [
  ...
  {
    ...sensors.withHttpIntegration[2],
    authorName: userprofiles[0].display_name || "",
    parsedRecords: parseSensorRecords(sensors.withHttpIntegration[2].records),
